...
def build_pre(numeric_cols: list[str], categorical_cols: list[str]) -> ColumnTransformer:
...
def prepare_fold(X: pd.DataFrame, y: pd.Series, train_idx, test_idx):
    X = X.copy()
    X = X.loc[:, X.notna().any()]
    ...
    pre = build_pre(numeric_cols, categorical_cols)
    pre.fit(X_train, y_train)
    return X_train, X_test, y_train, y_test, pre


def run_cv(alpha: float, n_splits: int, scenario: str) -> pd.DataFrame:
    X, y = load_data()
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(outer.split(X, y), start=1):
        X_train, X_test, y_train, y_test, pre = prepare_fold(X, y, train_idx, test_idx)

        for split_type in ["iid", "non_iid"]:
            seed = RANDOM_STATE + fold_idx * 100 + (0 if split_type == "iid" else 1)
            clients = make_clients(X_train, y_train, split_type, alpha, NUM_CLIENTS, seed)

            fed_lr = train_fedavg_lr(clients, pre)
            for m, v in evaluate_model(fed_lr, X_test, y_test).items():
                rows.append(
                    {
                        "scenario": scenario,
                        "split": split_type,
                        "model": "flower_fedavg_lr",
                        "metric": m,
                        "fold": fold_idx,
                        "value": v,
                    }
            ...
            fed_rf = train_fed_rf(clients, pre)
            for m, v in evaluate_model(fed_rf, X_test, y_test).items():
                rows.append(
                    {
                        "scenario": scenario,
                        "split": split_type,
            ...
            if FEDTREE_AVAILABLE:
                ...
                if fedtree is not None:
                    for m, v in evaluate_model(fedtree, X_test, y_test).items():
                        rows.append(
                            {
                                "scenario": scenario,
                                "split": split_type,
...
def main():
    ...
    parser.add_argument("--out", type=str, default=str(ROOT / "results" / "perturbation_iid_vs_noniid_cv.csv"))
    args = parser.parse_args()

    if not FEDTREE_AVAILABLE:
        print("Aviso: fedtree nao esta disponivel; pulando FedTree.")

    df = run_cv(alpha=args.alpha, n_splits=args.n_splits, scenario=args.scenario)
