...
def read_pred_csv(path: Path) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    df = pd.read_csv(path)
    return df["row_id"].to_numpy(), df["target"].to_numpy(dtype=int), df["probability"].to_numpy(dtype=float)
...
def main():
    ...
    parser.add_argument(
        "--central",
        default=str(root / "results" / "oof_predictions_all_nosmote_iid_central_rf.csv"),
    )
    parser.add_argument(
        "--fedavg",
        default=str(root / "results" / "oof_predictions_all_nosmote_iid_fedavg_lr.csv"),
    )
    parser.add_argument(
        "--fedrf",
        default=str(root / "results" / "oof_predictions_all_nosmote_iid_fed_rf_vote.csv"),
    )
    parser.add_argument(
        "--fedtree",
        default=str(root / "results" / "oof_predictions_all_nosmote_iid_fedtree_gbdt.csv"),
    )
    ...
    parser.add_argument(
        "--out-csv",
        default=str(root / "results" / "bootstrap_oof_metric_comparison.csv"),
    )
    parser.add_argument(
        "--out-plot",
        default=str(root / "results" / "bootstrap_oof_metric_comparison_plot.png"),
    )
    parser.add_argument(
        "--out-delong",
        default=str(root / "results" / "delong_roc_oof_comparison.csv"),
    )
    ...
    for name, p in paths.items():
        row_id, y_true, prob = read_pred_csv(p)
